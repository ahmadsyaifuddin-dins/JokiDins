...
const UserActivity = () => {
  ...
  const fetchActivities = async (pageToFetch) => {
    try {
      ...
      const newActivities = Array.isArray(res.data) ? res.data : [];

      // Kalau data yang diterima kurang dari limit, artinya tidak ada lagi data
      if (newActivities.length < 10) setHasMore(false);

      // Jika halaman pertama, replace, kalau bukan, append
      setActivities((prev) =>
        pageToFetch === 1 ? newActivities : [...prev, ...newActivities]
      );

      // Jika halaman pertama, juga ambil data user
      if (pageToFetch === 1) {
        try {
  ...
  useEffect(() => {
    // Reset halaman dan data saat id berubah
    setPage(1);
    setHasMore(true);
    setActivities([]);
    setLoading(true);
  ...
  const getActivityTypeClass = (activity) => {
    if (activity.description.includes("login"))
      return "bg-blue-100 border-blue-300";
    if (
      activity.description.includes("ubah") ||
      activity.description.includes("edit")
    )
      return "bg-yellow-100 border-yellow-300";
    if (activity.description.includes("hapus"))
      return "bg-red-100 border-red-300";
    if (
      activity.description.includes("tambah") ||
      activity.description.includes("buat")
    )
  ...
  // Filter dan sort dilakukan di frontend (optional)
  const filteredActivities = Array.isArray(activities)
    ? activities
        .filter((activity) => {
          ...
          if (
            filterType === "login" &&
            !activity.description.includes("login")
          ) {
            return false;
          } else if (
            filterType === "edit" &&
            ...
            filterType === "delete" &&
            !activity.description.includes("hapus")
          ) {
            return false;
          } else if (
            filterType === "create" &&
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {/* Activity List dengan Infinite Scroll */}
        <InfiniteScroll
          dataLength={activities.length}
          next={fetchMoreData}
          hasMore={hasMore}
          loader={<div className="text-center py-4">Memuat data...</div>}
          endMessage={
            <p className="text-center py-4">
              <b>Semua aktivitas sudah dimuat.</b>
            </p>
          }
        >
            ...
            <div className="bg-white rounded-lg shadow-md overflow-hidden">
              <div className="p-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                <h3 className="font-medium text-gray-700">
                  Riwayat Aktivitas ({filteredActivities.length})
                </h3>
              </div>
              <ul className="divide-y divide-gray-200">
                {filteredActivities.map((activity, index) => (
                  <li
                    ...
                    className={`p-4 hover:bg-gray-50 transition ${getActivityTypeClass(
                      activity
                    )} border-l-4`}
                  >
