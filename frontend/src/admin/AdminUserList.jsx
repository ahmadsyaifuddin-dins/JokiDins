...
const AdminUserList = () => {
    ...
    if (searchTerm) {
      result = result.filter(
        (user) =>
          ...
          (user.phones && user.phones.some(phone => 
  ...
  const handleDelete = async (userId) => {
    ...
    try {
      ...
      // Use toast notification instead of alert for better UI
    ...
    <div className="max-w-full mx-auto p-6 bg-white">
      <div className="bg-white shadow-lg rounded-xl overflow-hidden">
        <div className="bg-gradient-to-r from-blue-600 to-blue-800 p-6">
          ...
          <p className="text-blue-100">Kelola semua pengguna terdaftar dalam sistem</p>
        </div>
        ...
        {filteredUsers.length === 0 ? (
          <div className="p-8 text-center">
            <div className="text-gray-500 font-medium mb-2">Tidak ada data pengguna yang ditemukan</div>
            <p className="text-gray-400 text-sm">Coba ubah filter pencarian atau tambahkan pengguna baru</p>
          </div>
        ) : (
            ...
            <table className="min-w-full text-sm text-left">
              <thead className="bg-gray-100 text-gray-700">
                <tr>
                  ...
                  <th className="py-3 px-4 font-semibold cursor-pointer group" onClick={() => requestSort('name')}>
                    <div className="flex items-center">
                      Nama
                      <svg className={`w-4 h-4 ml-1 ${sortConfig.key === 'name' ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={sortConfig.key === 'name' && sortConfig.direction === 'descending' ? "M7 11l5 5 5-5M7 6l5 5 5-5" : "M7 16l5-5 5 5M7 11l5-5 5 5"}></path>
                      </svg>
                    </div>
                  </th>
                  <th className="py-3 px-4 font-semibold cursor-pointer group" onClick={() => requestSort('email')}>
                    <div className="flex items-center">
                      Email
                      <svg className={`w-4 h-4 ml-1 ${sortConfig.key === 'email' ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={sortConfig.key === 'email' && sortConfig.direction === 'descending' ? "M7 11l5 5 5-5M7 6l5 5 5-5" : "M7 16l5-5 5 5M7 11l5-5 5 5"}></path>
                      </svg>
                    </div>
                  </th>
                  <th className="py-3 px-4 font-semibold">Nomor HP</th>
                  <th className="py-3 px-4 font-semibold cursor-pointer group" onClick={() => requestSort('role')}>
                    <div className="flex items-center">
                      Role
                      <svg className={`w-4 h-4 ml-1 ${sortConfig.key === 'role' ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d={sortConfig.key === 'role' && sortConfig.direction === 'descending' ? "M7 11l5 5 5-5M7 6l5 5 5-5" : "M7 16l5-5 5 5M7 11l5-5 5 5"}></path>
                      </svg>
                    </div>
                  </th>
